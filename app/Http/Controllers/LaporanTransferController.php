<?php
    ...
    use App\Exports\LaporanTransferExport;
    ...
    class LaporanTransferController extends Controller
    {
        ...
        {
            // $laporanTransfer = LaporanTransfer::all()
        ...
        ->orderBy('created_at', 'desc')
            ...
            return view('admin.laporan_transfer.data', compact('laporanTransfer'));
        }

        public function showPaymentProof($id)
        {
            $laporanTransfer = LaporanTransfer::find($id);
            if (!$laporanTransfer || !$laporanTransfer->buktipembayaran) {
                return abort(404, 'Bukti pembayaran tidak ditemukan.');
            }
            return response()->file(storage_path("app/public/{$laporanTransfer->buktipembayaran}"));
    
        }   
        public function showSspdProof($id)
        {
            $laporanTransfer = LaporanTransfer::find($id);
            if (!$laporanTransfer || !$laporanTransfer->buktisspd) {
                return abort(404, 'Bukti sspd tidak ditemukan.');
            }
            return response()->file(storage_path("app/public/{$laporanTransfer->buktisspd}"));
...
//     if (empty($laporanTransfer->buktisspd)) {
//         return response()->json(['success' => false, 'message' => 'Bukti SSPD kosong.'], 400);
//     }

//     if ($laporanTransfer->konfirmasi !== 'Belum kirim') {
//         return response()->json(['success' => false, 'message' => 'Status sudah diperbarui.'], 400);
//     }

//     $laporanTransfer->konfirmasi = 'Sudah kirim';
//     $laporanTransfer->save();
...
public function updateKonfirmasi(Request $request, $id)
{
    try {
        ...
        $laporanTransfer = LaporanTransfer::findOrFail($id);
        $phoneNumber = $laporanTransfer->telepon;
        ...
        if ($jenisPesan) {
            ...
            $sspdUrl = $this->getSspdUrl($laporanTransfer->buktisspd);
            // $nama = $laporanTransfer->nama_pajak;
            // $tanggal = $laporanTransfer->tanggal_pembayaran;
            // $jumlah = $laporanTransfer->jumlah_pembayaran;
            // $metode = $laporanTransfer->metode_pembayaran;
...
public function filter(Request $request)
{
    ...
    $laporanTransfer = $query->latest()->get();
    ...
    return view('admin.laporan_transfer.data', compact('laporanTransfer'));
        ...
        {
            return Excel::download(new LaporanTransferExport, 'laporan_transfer.xlsx');
        }
        
        public function exportPdf(Request $request)
{
    // Mulai query untuk LaporanTransfer
    $query = LaporanTransfer::query();
    ...
    $pdf = Pdf::loadView('admin.laporan_transfer.pdf', compact('laporanTransfer'));

    // Unduh file PDF
    return $pdf->download('laporan_transfer.pdf');
}
    }