<?php
    ...
    class DataTransferController extends Controller
    {
        public function index(Request $request)
        {
            ...
            $dataTransfer = DataTransfer::where('zona', $zonaUser)
                ->where('metode_pembayaran', 'Transfer')
        ->orderBy('created_at', 'desc')

                ->get();
        
            return view('petugas_penagihan.data_transfer.data', compact('dataTransfer'));
        }
        ...
        {
            $dataTransfer = DataTransfer::find($id);
            if (!$dataTransfer || !$dataTransfer->buktipembayaran) {
                return abort(404, 'Bukti pembayaran tidak ditemukan.');
            }
            return response()->file(storage_path("app/public/{$dataTransfer->buktipembayaran}"));
    
        }   
        public function showSspdProof($id)
        {
            $dataTransfer = DataTransfer::find($id);
            if (!$dataTransfer || !$dataTransfer->buktisspd) {
                return abort(404, 'Bukti sspd tidak ditemukan.');
            }
            return response()->file(storage_path("app/public/{$dataTransfer->buktisspd}"));
...
public function uploadSspdInline(Request $request, $id)
{
    ...
    if ($filePath) {
        $dataTransfer->update(['buktisspd' => $filePath]);

        // Update laporan transfer
        $laporanTransfer = LaporanTransfer::where('npwpd', $dataTransfer->npwpd)
            ->where('periode', $dataTransfer->periode)
            ->first();
        if ($laporanTransfer) {
            $laporanTransfer->update(['buktisspd' => $filePath]);
        }
...
public function exportPdf(Request $request)
{
    ...
    $query = DataTransfer::where('zona', $zonaUser)->orderBy('created_at', 'desc');
    ...
    $dataTransfer = $query->get();

    // Kirim data ke view khusus untuk PDF
    $pdf = Pdf::loadView('petugas_penagihan.data_transfer.pdf', compact('dataTransfer'));

    // Unduh file PDF
    return $pdf->download('data_transfer_petugas.pdf');
}
    }