    ...
    public function collection()
{
    ...
    $data = DataPenetapan::with(['jenisPajak', 'kategoriPajak'])->get();

    // Pastikan data berjumlah 6 dan mapping data
    return $data->map(function ($penetapan) {
        return [
            $penetapan->nama_pajak,
            $penetapan->alamat,
            $penetapan->npwpd,
            $penetapan->jenisPajak->jenispajak ?? '-',
            $penetapan->kategoriPajak->kategoripajak ?? '-',
            $penetapan->jumlah_penagihan,
            $penetapan->periode,
        ];
    });
}
...
public function registerEvents(): array
{
    return [
        AfterSheet::class => function (AfterSheet $event) {
            ...
            $sheet->mergeCells('A1:G1'); // Menggabungkan sel A1 hingga G1
