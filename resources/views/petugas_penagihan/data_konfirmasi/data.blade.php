...
@endif
    ...
    <div class="box-header with-border">
        <h3 class="box-title">
            Data Konfirmasi (Zona {{ Auth::user()->zona ?? 'Tidak Ditentukan' }})
        </h3>
    </div>

    
    <div class="box-body">
        <form action="{{ route('petugas_penagihan.data_konfirmasi.filter') }}" method="GET" id="filterForm">
            <div class="row">
                <!-- Filter Pencarian -->
                <div class="col-md-3">
    ...
    <div class="box-body table-responsive">
        ...
        <thead>
            ...
            <tbody>
                @if($dataKonfirmasi->isEmpty())
                <tr>
                    <td colspan="14" class="text-center">Tidak ada data konfirmasi.</td>
                </tr>
                @else
                @foreach ($dataKonfirmasi as $key => $konfirmasi)
                <tr>
                    <td><b>{{ $key + 1 }}</td>
                    <td><b>{{ $konfirmasi->nama_pajak }}</td>
                    <td><b>{{ $konfirmasi->alamat }}</td>
                    <td><b>{{ $konfirmasi->npwpd }}</td>
                    <td><b>{{ $konfirmasi->jenisPajak->jenispajak ?? 'N/A' }}</td>
                    {{-- <td><b>{{ $pelunasan->kategoriPajak->kategoripajak ?? 'N/A' }}</td>                     --}}
                    <td><b>{{ $konfirmasi->telepon }}</td>
                    {{-- <td><b>{{ $pelunasan->zona }}</td> --}}
                    {{-- <td><b>Rp{{ number_format((float) $pelunasan->tagihan, 0, ',', '.') }}</td> --}}
                    <td><b>{{ $konfirmasi->periode }}</td>

                    <td><b>{{ \Carbon\Carbon::parse($konfirmasi->tanggal_kunjungan)->locale('id')->isoFormat('D MMMM YYYY') }}</td>

                    {{-- <td><b>{{ $konfirmasi->metode_pembayaran }}</td> --}}
                    {{-- <td><b>{{ $konfirmasi->jumlah_pembayaran }}</td> --}}
                    ...
                    <td><b>
                        ...
                        onclick="showImageProof('{{ route('petugas_penagihan.data_konfirmasi.showVisitProof', $konfirmasi->id) }}', 'Bukti Visit')">
                        <i class="fa-solid fa-eye"></i> Lihat
                    </a>
                    {{-- <a href="{{ Storage::url($konfirmasi->buktivisit) }}" 
                        target="_blank" 
                        class="btn btn-warning btn-xs">
                        <i class="fa-solid fa-eye"></i> Lihat
                     </a> --}}
            ...
            {{-- <td><b>{{ $konfirmasi->keterangan }}</td> --}}
            <td><b>{{ $konfirmasi->keterangan ?? 'Tidak ada' }}</td>
    ...
    <div class="box-footer text-left">
        ...
        <a href="{{ route('petugas_penagihan.data_konfirmasi.exportPdf', [
