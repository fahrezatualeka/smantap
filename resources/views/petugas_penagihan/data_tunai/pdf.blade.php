...
<head>
    ...
    <title>Data Tunai Petugas Penagihan BAPENDA Kota Ambon</title>
    <style>
                @page {
            size: A4 landscape;
            margin: 5mm;
...
<body>
    <h2>Data Tunai Petugas Penagihan Zona {{ Auth::user()->zona ?? 'Tidak Ditentukan' }} BAPENDA Kota Ambon</h2>
    <table>
        <thead>
            <tr>
                <th >No</th>
        ...
        <tbody>
            @foreach($dataTunai as $index => $tunai)
                <tr>
                    <td >{{ $index + 1 }}</td>
