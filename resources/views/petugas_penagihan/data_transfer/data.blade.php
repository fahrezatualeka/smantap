...
@endif
    ...
    <div class="box-header with-border">
        <h3 class="box-title">
            Data Transfer Wajib Pajak (Zona {{ Auth::user()->zona ?? 'Tidak Ditentukan' }})
        </h3>
    </div>
    ...
    <div class="box-body">
        <form action="{{ route('petugas_penagihan.data_transfer.filter') }}" method="GET" id="filterForm">
            <div class="row">
                <!-- Filter Pencarian -->
                <div class="col-md-3">
    ...
    <div class="box-body table-responsive">
        ...
        <thead>
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="14" class="text-center">Tidak ada data transfer.</td>
                </tr>
                @else
                @foreach ($dataTransfer as $key => $transfer)
                <tr>
                    <td><b>{{ $key + 1 }}</td>
                    <td><b>{{ $transfer->nama_pajak }}</td>
                    <td><b>{{ $transfer->alamat }}</td>
                    <td><b>{{ $transfer->npwpd }}</td>
                    <td><b>{{ $transfer->jenisPajak->jenispajak ?? 'N/A' }}</td>
                    {{-- <td><b>{{ $pelunasan->kategoriPajak->kategoripajak ?? 'N/A' }}</td>                     --}}
                    <td><b>{{ $transfer->telepon }}</td>
                    {{-- <td><b>{{ $pelunasan->zona }}</td> --}}
                    {{-- <td><b>Rp{{ number_format((float) $pelunasan->tagihan, 0, ',', '.') }}</td> --}}
                    <td><b>{{ $transfer->periode }}</td>
                    <td><b>{{ \Carbon\Carbon::parse($transfer->tanggal_pembayaran)->locale('id')->isoFormat('D MMMM YYYY') }}</td>
                    
                    <td><b>Rp{{ number_format((float) $transfer->jumlah_pembayaran, 0, ',', '.') }}</td>
                    
                <td><b>
                    @if($transfer->buktipembayaran)
                    <a href="javascript:void(0);" 
                    class="btn btn-warning btn-xs" 
                    onclick="showImageProof('{{ route('petugas_penagihan.data_transfer.showPaymentProof', $transfer->id) }}', 'Bukti Pembayaran')">
            ...
            <td><b>
                @if($transfer->buktisspd)
                    <a href="javascript:void(0);" 
                       class="btn btn-warning btn-xs" 
                       onclick="showImageProof('{{ route('petugas_penagihan.data_transfer.showSspdProof', $transfer->id) }}', 'Bukti SSPD')">
                       <i class="fa-solid fa-eye"></i> Lihat
                    </a>
                @else
                    <span class="text-danger">Tidak ada</span><br>
                    <button class="btn btn-default btn-xs" data-toggle="modal" data-target="#uploadModal-{{ $transfer->id }}">
                        <i class="fa-solid fa-upload"></i> Foto SSPD
                    </button>
            
                    <!-- Modal -->
                    <div class="modal fade" id="uploadModal-{{ $transfer->id }}" tabindex="-1" role="dialog" aria-labelledby="uploadModalLabel-{{ $transfer->id }}" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title text-center" id="paymentModalLabel{{ $transfer->id }}">
                                        <b>Form Pengiriman Bukti SSPD Data Transfer</b>
                                        <p>Wajib Pajak {{ $transfer->nama_pajak }}</p>
                                    </h4>
                                </div>
                                <form action="{{ route('petugas_penagihan.data_transfer.uploadSspdInline', $transfer->id) }}" method="POST">
                                    @csrf
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <label for="nama_pajak">Nama Pajak</label>
                                            <input type="text" name="nama_pajak" id="nama_pajak" class="form-control" value="{{ $transfer->nama_pajak }}" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label for="npwpd">NPWPD</label>
                                            <input type="text" name="npwpd" id="npwpd" class="form-control" value="{{ $transfer->npwpd }}" readonly>
                                        </div>

                                        <div class="form-group">
                                            <label>Bukti SSPD</label>
                                            <br>
                                            <button id="take-photo-btn-{{ $transfer->id }}" type="button" class="btn btn-secondary" onclick="openWebcam({{ $transfer->id }})">
                                                <i class="fa fa-camera"></i> Foto
                                            </button>
                                            
                                            <div id="webcam-container-{{ $transfer->id }}" style="display:none; position: relative;">
                                                <video id="video-webcam-{{ $transfer->id }}-sspd" autoplay style="width: 100%; max-width: 400px; border: 2px solid #ccc;"></video><br>
                                                
                                                <!-- Tombol rotasi kamera (ikon) -->
                                                <button id="rotate-camera-btn-{{ $transfer->id }}" class="btn btn-secondary" type="button" style="position: absolute; top: 10px; right: 10px; z-index: 10;" onclick="rotateCamera({{ $transfer->id }})">
                                                    <i class="fa fa-sync"></i>
                                                </button>
                                            
                                                <button id="capture-photo-{{ $transfer->id }}-sspd" class="btn btn-warning" type="button" style="margin-top: 10px;" onclick="capturePhoto({{ $transfer->id }})">
                                                    <i class="fa fa-camera"></i> Ambil Gambar
                                                </button>
                                            </div>
                                            <div id="photo-preview-container-{{ $transfer->id }}" style="display:none;">
                                                <img id="photo-preview-{{ $transfer->id }}" src="" alt="Preview Foto" style="width: 100%; max-width: 400px; margin-top: 10px;"/>
                                                <button id="retake-photo-btn-{{ $transfer->id }}" class="btn btn-info" type="button" style="margin-top: 10px;" onclick="retakePhoto({{ $transfer->id }})">
                                                    <i class="fa fa-repeat"></i> Foto Ulang
                                                </button>
                                            </div>
                                            <input type="hidden" name="photo_data" id="photo-data-{{ $transfer->id }}">
                                        </div>
            ...
            <td><b>{{ $transfer->keterangan ?? 'Tidak ada' }}</td>
    ...
    <div class="box-footer text-left">
        ...
        <a href="{{ route('petugas_penagihan.data_transfer.exportPdf', [
