...
<head>
    ...
    <title>Laporan Penutupan BAPENDA Kota Ambon</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 5mm;
...
<body>
    <h2>Laporan Penutupan BAPENDA Kota Ambon</h2>
    <table>
        <thead>
            <tr>
                <th >No</th>
        ...
        <tbody>
            @foreach($laporanPenutupan as $index => $penutupan)
                <tr>
                    <td >{{ $index + 1 }}</td>
                    ...
                    <td>{{ \Carbon\Carbon::parse($penutupan->tanggal_pembayaran)->locale('id')->isoFormat('D MMMM YYYY') }}</td>
