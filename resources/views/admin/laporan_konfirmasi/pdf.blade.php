...
<head>
    ...
    <title>Laporan Konfirmasi BAPENDA Kota Ambon</title>
    <style>
        @page {
            size: A4 landscape;
            margin: 5mm;
...
<body>
    <h2>Laporan Konfirmasi BAPENDA Kota Ambon</h2>
    <table>
        <thead>
            <tr>
                <th >No</th>
        ...
        <tbody>
            @foreach($laporanKonfirmasi as $index => $konfirmasi)
                <tr>
                    <td >{{ $index + 1 }}</td>
                    ...
                    <td>{{ \Carbon\Carbon::parse($konfirmasi->tanggal_pembayaran)->locale('id')->isoFormat('D MMMM YYYY') }}</td>
